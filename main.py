...
# ── CRÍTICO: agregar raíz del proyecto al path PRIMERO ────────────
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
if BASE_DIR not in sys.path:
    sys.path.insert(0, BASE_DIR)
# También cambiar el directorio de trabajo a la raíz
os.chdir(BASE_DIR)
...
class BarraTitulo:
    ...
    def _construir(self):
        self._lbl_icono = tk.Label(self._barra, text="🏗", font=("Segoe UI", 22))
        ...
        self._lbl_ver = tk.Label(self._barra, text="v2.0 | SQLite", font=("Segoe UI", 8))
    ...
    def _refrescar(self):
        ...
        for nombre, btn in self._botones_tema.items():
            if nombre == activo:
                btn.configure(bg=tm().color("primario_lt"), fg=tm().color("texto_inv"), relief="solid")
            else:
