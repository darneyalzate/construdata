...
class VistaProyectos(VistaBase):
    ...
    def __init__(self, notebook: ttk.Notebook):
        ...
        self._widgets_tema = []   # lista de (widget, clave_bg, clave_fg)
        self._construir_ui()
    ...
    def _refrescar_tema(self):
        """Llamado por ThemeManager al cambiar el tema."""
    ...
    def _leer_campos(self):
        def gv(w):
            if isinstance(w, ttk.Combobox):
                return w.get()
            try:
                return w.get_date().strftime("%Y-%m-%d") if CALENDAR_OK else w.get()
        ...
        return dict(
            ...
            estado=gv(self.e_est),
            gerente=self.e_ger.get().strip(),
            area=self.e_area.get().strip(),
            pisos=self.e_pisos.get().strip(),
            pre=self.e_pre.get().strip(),
            f_ini=gv(self.e_fini),
            f_fin=gv(self.e_ffin),
        )
    ...
    def _exportar_excel(self):
        ...
        if not filas:
            self.advertencia("No hay datos para exportar."); return
